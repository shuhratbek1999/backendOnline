...
const kitobModel = require('../../models/kitob.model');

class kompyuterController {
    getAll = async(req, res, next) => {
        const model = await kitobModel.findAll(req.body)
        res.send({
            error_code: 201,
            error: false,
     ...
     getOne = async(req, res, next) => {
        const model = await kitobModel.findOne({
            where:{
                id: req.params.id
            }
        })
   ...
   create = async(req, res, next) =>{
    ...
    const model = await kitobModel.create(req.body);
    if(!model){
  ...
  delete = async(req,res, next) => {
    const model = await kitobModel.destroy({
        where:{
            id: req.params.id
        }
    })
